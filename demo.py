...
alerts = []
blacklist_ips = set()
quarantine_ips = defaultdict(int)
...
# ===============================
# 5. Demo Mode (Fake Alerts)
# ===============================
def demo_mode():
    demo_ips = ["192.168.1.50", "45.133.1.10", "10.0.0.99"]
    demo_reasons = [
        "Suspicious port 4444 accessed",
        "Blacklisted IP detected",
        "Possible brute force/scan detected"
    ]

    for i in range(3):
        handle_threat(demo_ips[i], demo_reasons[i])
        time.sleep(2)

    print("\n[INFO] Demo complete. Check alerts above or in alerts.log.\n")
...
def console_dashboard():
    ...
    ascii_banner = pyfiglet.figlet_format("Z+TDS", font="slant")  # change font here
    print(ascii_banner)
    print("\nRecent Alerts:")
...
def main():
    ...
    parser.add_argument("--demo", action="store_true", help="Run demo mode with fake alerts")
    ...
    if args.run:
        try:
            ...
            print("\n[INFO] Stopping Threat Detection System.")
